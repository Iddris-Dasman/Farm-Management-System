   ...
   if(isset($_GET['sales_id']))
    {
      ...
      // SQL query to select all the data from the table where id = $sales
      $query="SELECT * FROM sales WHERE id = $salesid ";
      $view_sales= mysqli_query($conn,$query);
    ...
    if(isset($_POST['updatesales'])) 
    {
      ...
      // SQL query to update the data in sales table where the id = $sales 
      $query = "UPDATE sales SET date = '{$date}' , item = '{$item}' , ucost = '{$ucost}', quantity = '{$quantity}', tcost = '{$tcost}' WHERE id = $salesid";
      $update_crop = mysqli_query($conn, $query);
      echo "<script type='text/javascript'>alert('sales data updated successfully!')</script>";
    }             
?>

<h1 class="text-center">Edit Finance </h1>
  <div class="container ">
    <form action="" method="post">
      ...
      <div class="form-group">
         <input type="submit"  name="updatesales" class="btn btn-secondary mt-2" value="Update Sales">
      </div>
      </div>
    </form>    
  </div>

    <!-- a BACK button to go to the sales page -->
